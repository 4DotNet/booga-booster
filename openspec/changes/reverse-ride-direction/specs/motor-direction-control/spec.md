...
WHEN a motor group's direction is reversed while its body is already rotating,
the drive torque SHALL oppose the current motion. The body SHALL first slow
down, pass through a standstill, and then accelerate in the newly commanded
direction. The reversal SHALL be continuous — the rotation speed MUST pass
through zero rather than jump discontinuously to the opposite sign.
