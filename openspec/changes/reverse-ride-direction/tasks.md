...
- [ ] 7.2 Manually verify in the running app: start the ride, spin up, toggle Reverse — the mill/hubs slow, stop, and turn the other way in the 3D view and gauges. (Requires running the Aspire host + `ng serve`; left for a human run. Behaviour is covered by the automated domain/telemetry/frontend suites above.)
