...
- [ ] 5.2 Manual/Aspire smoke check: with the queue filling, drive `Idle → Loading` and confirm groups drain into seats until the ride reports full, and a group arriving mid-loading boards immediately. _Deferred — requires running the Aspire AppHost with the Dapr sidecars + RabbitMQ; a manual step for the user._
