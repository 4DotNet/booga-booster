...
- The workflow installs the **GitHub Copilot CLI** (`@github/copilot`, pinned) on Node 22 and runs it **non-interactively** (`copilot -p …`) over a **deny-by-default tool allowlist** — no edits to tracked files, no pushes, no network writes.
...
- The CLI's **built-in GitHub MCP server is disabled** for the review, and the repository's `.mcp.json` servers are not started: the reviewer works from a diff already on disk and needs no network tools.
...
- **Cost** — each review consumes **Copilot premium requests** from the seat holder's monthly allowance rather than metered API tokens. The workflow bounds this with per-PR concurrency cancellation, a draft skip, `paths-ignore`, and a job timeout; a shared personal allowance is the main reason to prefer a dedicated machine account.
