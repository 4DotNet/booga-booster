...
- [ ] 8.2 Manually verify in the running app: start and spin up the ride, press "Apply brakes" — the sliders disable and the ride stops within a couple of seconds; release — the sliders re-enable; press Stop — the ride brakes to rest fast and offloads. (Requires the Aspire host + `ng serve`; left for a human run. Behaviour is covered by the automated domain/physics/telemetry and frontend suites above.)
