...
- [ ] 10.2 Run the backend via the Aspire AppHost and manually exercise `GET /weather`, `POST /weather/precipitation`, and `POST /weather/strong-wind`
- [ ] 10.3 Confirm `WeatherUpdateIntegrationEvent` messages are published on weather changes (observe via the RabbitMQ management plugin / a test subscriber)
