# ride-queue Specification

## Purpose

Defines the per-ride queue domain model: enqueuing arriving guests and groups, preserving arrival order and group membership, exposing the next group to board, publishing a group-queued integration event, and tracking the queue's lifecycle state.
