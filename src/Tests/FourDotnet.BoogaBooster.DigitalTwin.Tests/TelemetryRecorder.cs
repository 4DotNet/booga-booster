...
internal sealed class TelemetryRecorder : IDisposable
{
    ...
    private ActivitySpanId _scopeSpan;
    ...
    /// <summary>
    /// Every completed activity in the open scope, whatever its name — the scope
    /// activity itself excluded. Lets a test assert that some work started none at all.
    /// </summary>
    internal List<Activity> ActivitiesInScope()
    {
        lock (_activities)
        {
            return
            [
                .. _activities.Where(activity =>
                    activity.TraceId == _scopedTrace && activity.SpanId != _scopeSpan),
            ];
        }
    }
